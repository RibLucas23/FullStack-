...
export default function CartContainer() {
   ...
   useEffect(() => {
      const fetchProducts = async () => {
         ...
         try {
            ...
            const response = await fetch(`http://localhost:8080/api/carts/mongo/${user.cart}`, {
               method: 'GET',
               headers: {
   ...
   const finishBuy = async () => {
      try {
         // Realizar la solicitud solo si el usuario está autenticado
         const response = await fetch(`http://localhost:8080/api/carts/mongo/${user.cart}/purchase`, {
            method: 'POST',
            headers: {
               'Content-Type': 'application/json',
            }, // Incluye las credenciales en la solicitud
